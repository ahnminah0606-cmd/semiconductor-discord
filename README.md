...
* 사이트별 최신 기사 5개의 한국어 제목·요약을 하나의 Discord Embed 메시지로 전송
* 기사별 링크 알림을 보내지 않고 사이트별 일일 요약 메시지로 통합
...
실행에서 같은 기사들을 다시 시도할 수 있습니다. API 또는 Discord 발송 오류가
...
사이트별 기사 5건의 한국어 요약 제목과 핵심 내용을 하나의 Discord 메시지 안에서
각각 Embed 카드로 표시합니다. 메시지당 Embed 10개 및 전체 텍스트 6,000자 제한에
여유를 둔 5,800자를 기준으로 하며, 이를 넘을 때만 `(계속)` 메시지로 분할합니다.
